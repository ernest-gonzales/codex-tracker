...
fn parse_file(
    ...
    let mut reader = BufReader::new(file);
    let mut buf = String::new();
    let session_id = session_id_from_source(&task.file_path);
...
pub fn ingest_codex_home(db: &mut Db, codex_home: &Path) -> Result<IngestStats> {
    ...
    let parsed_files: Vec<ParsedFile> = tasks
        .into_par_iter()
        .map(|task| parse_file(task, pricing.as_ref(), has_pricing, timing_enabled))
        .collect();
